...
export function inyectarBarraBusqueda() {
  ...
  let searchInput = document.getElementById('buscador-flota');
  if (!searchInput) {
    ...
    searchInput.style.cssText = 'width: 100%; max-width: 280px; padding: 6px 10px; margin-top: 10px; margin-bottom: 10px; font-size: 0.78rem; border-radius: 8px; border: 1px solid var(--border); background: var(--bg); color: var(--text); outline: none; transition: border-color 0.2s;';
    
    searchInput.onfocus = () => { searchInput.style.borderColor = 'var(--primary)'; };
    searchInput.onblur = () => { searchInput.style.borderColor = 'var(--border)'; };

    if (kpisBar) {
      kpisBar.after(searchInput);
    } else {
      tableContainer.after(searchInput);
    }
  }
}
