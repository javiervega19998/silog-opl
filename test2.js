...
async function loadFilters(){
  try{
    ...
    vSnap.forEach(d=>{const v=d.data();const o=document.createElement('option');o.value=v.patente||d.id;o.textContent=v.patente||d.id;vSel.appendChild(o);});
    const uSel=document.getElementById('cc-conductor');
    uSnap.forEach(d=>{const u=d.data();if((u.rol||'').toLowerCase().includes('conductor')){const o=document.createElement('option');o.value=u.correo_electronico||u.email||'';o.textContent=u.nombre||u.name||u.correo_electronico||'';uSel.appendChild(o);}});
  ...
  try{
    ...
    if(!_allCostos.length){body.innerHTML='<tr><td colspan="8" class="txt-c empty">Sin datos para este periodo</td></tr>';
      ...
      body.innerHTML=_allCostos.map(r=>{
        ...
        const f=r._fecha?r._fecha.toLocaleDateString('es-CL',{day:'2-digit',month:'short'}):'â€”';
        return`<tr>
          <td>${f}</td>
          <td><code style="color:var(--accent)">${sanitize(r.patente||'â€”')}</code></td>
...
function renderHojasRuta(){
  ...
  const estadoBadge={'pendiente_revision':'<span class="badge-sm b-borrador">ð¡ Pendiente</span>','revisada':'<span class="badge-sm b-pagada">ð¢ Revisada</span>','observada':'<span class="badge-sm b-enviada">ð´ Observada</span>'};
  body.innerHTML=_hojasRuta.map(h=>`<tr>
    <td>${sanitize(h.fecha||'â')}</td>
    <td>${sanitize(h.conductor_nombre||h.conductor_email||'â')}</td>
    <td>${sanitize(h.distribuidor||'â')}</td>
    <td style="font-weight:700;color:var(--accent)">${sanitize(h.patente||'â')}</td>
    ...
    <td class="txt-c">${h.km_recorridos||'â'} km</td>
    <td class="txt-c">${estadoBadge[h.estado]||sanitize(h.estado)}</td>
    <td class="txt-c"><button class="btn-sm" onclick="openHoja('${sanitize(h.id)}')">ðï¸ Ver</button> <button class="btn-sm" style="background:var(--success);border-color:var(--success);color:#fff" onclick="exportHojaExcelById('${sanitize(h.id)}')">ð¥</button></td>
  </tr>`).join('');
}
function openHoja(id){
  _hojaActual=_hojasRuta.find(h=>h.id===id);if(!_hojaActual)return;
  document.getElementById('hoja-id').value=id;
  document.getElementById('hoja-conductor').textContent=_hojaActual.conductor_nombre||_hojaActual.conductor_email||'-';
  document.getElementById('hoja-patente').textContent=`${_hojaActual.patente||'-'} Â· ${_hojaActual.distribuidor||'Sin distribuidor'}`;
  document.getElementById('hoja-fecha').textContent=_hojaActual.fecha||'-';
  const hi=_hojaActual.hora_inicio;const ht=_hojaActual.hora_termino;
  document.getElementById('hoja-horario').textContent=`${hi?formatTime(hi):'-'} - ${ht?formatTime(ht):'-'} Â· KM: ${_hojaActual.km_inicial||0}-${_hojaActual.km_final||0} (${_hojaActual.km_recorridos||0} km)`;
  
  document.getElementById('hoja-combustible').value = _hojaActual.combustible || 0;
  document.getElementById('hoja-peaje').value = _hojaActual.peaje || 0;
  
  document.getElementById('hoja-estado').value=_hojaActual.estado||'pendiente_revision';
  ...
  tbody.innerHTML=entregas.map((e,i)=>`<tr>
    ...
    <td>
      <select class="field he-estado" style="width:125px; padding:6px 10px; font-size:0.8rem; border-radius:6px; font-weight:600; cursor:pointer; outline:none; transition:0.2s; color:${e.estado === 'entregado' || e.estado === 'Conforme' ? 'var(--success)' : 'var(--danger)'}; border-color:${e.estado === 'entregado' || e.estado === 'Conforme' ? 'rgba(16, 185, 129, 0.4)' : 'rgba(239, 68, 68, 0.4)'}; background:var(--surface2);" onchange="this.style.color = this.value === 'entregado' ? 'var(--success)' : 'var(--danger)'; this.style.borderColor = this.value === 'entregado' ? 'rgba(16, 185, 129, 0.4)' : 'rgba(239, 68, 68, 0.4)';">
        <option value="entregado" ${e.estado === 'entregado' || e.estado === 'Conforme' ? 'selected' : ''} style="color:var(--text);background:var(--bg);">ð¢ Entregado</option>
        <option value="devuelto" ${e.estado === 'devuelto' || e.estado === 'rechazado' || (e.estado !== 'entregado' && e.estado !== 'Conforme' && e.estado) ? 'selected' : ''} style="color:var(--text);background:var(--bg);">ð´ Rechazado</option>
      </select>
    </td>
...
async function guardarHoja(){
  ...
  const combustibleEditado = parseFloat(document.getElementById('hoja-combustible').value)||0;
  const peajeEditado = parseFloat(document.getElementById('hoja-peaje').value)||0;
  ...
  try{
    ...
    // Sync con Gastos en Ruta para el Centro de Costos
    if(_hojaActual && _hojaActual.turno_id) {
      ...
      if(combustibleEditado > 0) {
        if(combustibleDoc) batch.update(combustibleDoc.ref, { monto_clp: combustibleEditado, monto: combustibleEditado });
        else {
    ...
    if(_hojaActual && _hojaActual.turno_id) {
      ...
      entregas.forEach((e) => {
        ...
        const updateData = {
          ...
          distribuidor: _hojaActual.distribuidor || _hojaActual.nombre_distribuidor || ''
        };
        ...
        if (dMatchKey) {
          ...
          batch.set(newD, {
            ...updateData,
            turno_id: _hojaActual.turno_id,
            fecha: _hojaActual.fecha ? firebase.firestore.Timestamp.fromDate(new Date(_hojaActual.fecha + 'T12:00:00')) : firebase.firestore.FieldValue.serverTimestamp(),
